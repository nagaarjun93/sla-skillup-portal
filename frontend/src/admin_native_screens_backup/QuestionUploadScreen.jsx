...
export default function QuestionUploadScreen() {
  ...
  const handleBulkSubmit = async () => {
    ...
    try {
      ...
      const activeTopicObj = ALL_TOPICS.find(t => t.label === selectedTopicLabel) || { label: selectedTopicLabel, category: 'Vedic Math' };
      const categoryName = activeTopicObj.category;
      const topicLabel = activeTopicObj.label;

      lines.forEach((line, idx) => {
        if (idx === 0 && line.toLowerCase().startsWith('question')) return; // Skip header row if present
        ...
        if (parts.length >= 6) {
          questions.push({
            ...
            category: parts[6] || categoryName || (activeTab === 'mock' ? 'Mock Test' : 'General'),
            topic: parts[7] || topicLabel || (activeTab === 'mock' ? 'Official Mock' : 'General'),
            modelSet: activeTab === 'mock' ? (parts[8] || selectedMockModel) : 'Model 1',
      ...
      if (questions.length === 0) {
        const formatErrMsg = 'No valid questions parsed. Please ensure each line has at least 6 comma-separated fields: questionText, optionA, optionB, optionC, optionD, correctAnswer (A/B/C/D)';
