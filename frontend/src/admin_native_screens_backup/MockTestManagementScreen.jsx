...
export default function MockTestManagementScreen() {
  ...
  const handleBulkMockCsv = async () => {
    ...
    lines.forEach((line, idx) => {
      if (idx === 0 && line.toLowerCase().startsWith('question')) return; // skip header
      ...
      if (p.length >= 6 && p[0]) {
        questions.push({
          ...
          optionC: p[3] || '',
          optionD: p[4] || '',
          correctAnswer: (p[5] || 'A').toUpperCase(),
          category: 'Mock Test',
          topic: p[7] || 'Official Mock',
          modelSet: p[8] || selectedUploadModel
        });
      }
    });

    if (questions.length === 0) {
      Alert.alert('Format Error', 'No valid questions parsed. Format: question,optA,optB,optC,optD,correctAnswer(A/B/C/D)');
      return;
    }
